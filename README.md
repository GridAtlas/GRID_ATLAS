...
- 結界モード（z=18タイルの結界石、選択順で張る結界、任意の守護点、結界力の表示、結界画像の共有・PNG保存、配置イベント履歴）
- 結界モード龍脈眼（結界師クラスによる図形・辺長合計上限・頂点ばらつきの解放、グリッド上の移動・ピンチ拡縮、頂点地点の端末内自動保存）
- 結界・図形の長押しクイック操作（結界は「解く」、累積結界霊量は発動中に表示更新）
...
現在のWeb版は `0.2542` です。4桁の小数で、更新規模を次のように管理します。
