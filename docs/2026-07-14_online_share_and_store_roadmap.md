...
費用感としては、写真共有なし、JSON共有中心なら、当面は無料枠または低額で足りる可能性が高い。写真共有を標準化すると、保存容量とアクセス回数が効いてくるため、MVPでは写真を共有ペイロードに含めない。
...
- 共有範囲: 選択対象 / 表示中全体
- 含める内容: 地点、線、観察記録
- 写真を含める: MVPでは項目自体を出さない、またはOFF固定で無効
- コメント/メモを含める: MVPでは項目自体を出さない、またはOFF固定で無効
- 期限: 1日 / 7日 / 30日
- 注意文: 位置情報を含むリンクです
- 地点名一覧: 必ず表示
- 警告: 自宅、実家、職場などの危険ワードがあれば表示
- 警告: 現在地付近の地点があれば表示
- 確認: 未選択で全体共有する場合は追加確認
- 実行: 共有リンク作成
...
リンクを知っている人は内容を読み込めます。写真とコメントは初期版では含めません。
```

### 8.1.1 誤共有防止チェック

共有リンク作成前に、必ず以下をチェックする。

#### 地点名一覧の表示

共有対象に含まれる地点名を一覧表示する。地点数が多い場合も、先頭数件だけでなく展開して全件確認できるUIを用意する。

理由: `自宅`、`実家`、`職場` などをユーザーが目視で気づけるようにするため。

#### 危険ワード警告

地点名、将来コメント共有を許可する場合はコメント本文に、以下の語が含まれる場合は警告を出す。
...
```text
共有対象に私的な場所と思われる名前が含まれています。
このリンクを受け取った人は地点の位置を読み込めます。
```

危険ワードがある場合でも共有自体は禁止しない。ただし、明示的に確認させる。

#### 現在地付近警告

現在地が取得済みの場合、共有対象地点のいずれかが現在地から一定距離以内なら警告する。
...
```text
現在地付近の地点が共有対象に含まれています。
自宅や滞在場所を誤って共有していないか確認してください。
```

#### 全体共有の追加確認

未選択状態で共有リンクを作る場合、表示中の全データ共有になるため追加確認を入れる。

文言案:

```text
選択対象がありません。登録地点、線、観察記録をまとめて共有しますか？
私的な地点が含まれていないか確認してください。
```

#### 高精度座標の警告

GRID ATLASの共有は、原則として高精度な座標を含む。共有前モーダルに「正確な位置情報を含む」ことを明示する。
...
MVPでは写真とコメント/メモを共有ペイロードに含めない。

理由:

- 写真には人物、車、家、看板、EXIFなどが含まれやすい
- コメントには住所、個人名、生活情報、私的メモが含まれやすい
- EXIF除去や本文マスキングが必要になる
- 容量課金に影響する
- ストア審査上の説明が増える
...
- 取得するデータ: 位置情報、登録地点、観察記録。写真とコメントは端末内機能として残す場合でも、MVP共有には含めない
