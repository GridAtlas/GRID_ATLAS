...
param(
    ...
    [string]$OutputPath = (Join-Path (Get-Location) 'docs\data\candidate-sites-500-v4\kinki-shrine-temple-sites-v4-500.gridatlas')
)
...
foreach ($row in $rows) {
    ...
    $places += [ordered]@{
        id = ('kinki-shrine-temple-sites-v4-{0:d3}' -f [int]$row.no)
...
$document = [ordered]@{
    type = 'place-list'
    schemaVersion = 1
    id = 'kinki-shrine-temple-sites-v4-500'
    name = '公式データ 神社・寺院500地点 v4'
    description = '近畿五芒星の固定5地点と、文化庁 国指定文化財等データベース由来の神社建築・寺院建築495地点。'
    attribution = [ordered]@{
...
$tempDir = Join-Path ([IO.Path]::GetTempPath()) ('gridatlas-500-' + [Guid]::NewGuid().ToString('N'))
